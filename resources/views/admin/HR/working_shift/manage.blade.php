                ...
                <div class="card">
                    ...
                    <div class="card-body table-responsive" id="institue">
                        <form action="" class="es-form es-add-form">
                            <a href="{{route('add_workingshift')}}" class="btn btn-primary"><i class="fa-solid fa-plus"></i></a>
                            ...
                            <table class="table table-bordered mt-3 text-center branch_table ">
                                ...
                                @foreach($workingshifts as $workingshift)
                                    <tr>
                                        ...
                                        <td>{{$workingshift->name}}</td>
                                        <td>{{$workingshift->start_time}}</td>
                                        <td>{{$workingshift->end_time}}</td>
                                        <td>{{$workingshift->green_limit}}</td>
                                        <td>{{$workingshift->orange_limit}}</td>
                                        <td>{{$workingshift->red_limit}}</td>
                                        <td>{{$workingshift->status ==0 ? "Inactive":"Active"}}</td>
                                        <td>
                                            <a href="{{route('view_workingshift',$workingshift->id)}}"><i class="fa-solid fa-eye"></i></a>&nbsp &nbsp
                                            <a href="{{route('edit_workingshift',$workingshift->id)}}"><i class="fa-solid fa-pencil"></i></a>&nbsp &nbsp
                                            <a href="{{route('delete_workingshift',$workingshift->id)}}"><i class="fa-solid fa-trash"></i></a>&nbsp &nbsp
                                        </td>
                                    </tr>
                                @endforeach
