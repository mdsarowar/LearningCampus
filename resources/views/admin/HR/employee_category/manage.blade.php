                ...
                <div class="card">
                    ...
                    <div class="card-body table-responsive" id="institue">
                        <form action="" class="es-form es-add-form">
                            <a href="{{route('add_employee_category')}}" class="btn btn-primary"><i class="fa-solid fa-plus"></i></a>
...
{{--                                </tr>--}}
                                @foreach($emp_types as $emp_type)
                                    <tr>
                                        ...
                                        <td>{{$emp_type->type}}</td>
                                        <td>{{$emp_type->name}}</td>
                                        <td>{{$emp_type->dsoa==0 ? 'No':'Yes'}}</td>
                                        <td>{{$emp_type->hot==0 ? 'No':'Yes'}}</td>
                                        <td>{{$emp_type->status==0 ? 'Inactive':'Active'}}</td>
                                        <td>
                                            <a href="{{route('view_employee_category',$emp_type->id)}}"><i class="fa-solid fa-eye"></i></a>&nbsp &nbsp
                                            <a href="{{route('edit_employee_category',$emp_type->id)}}"><i class="fa-solid fa-pencil"></i></a>&nbsp &nbsp
                                            <a href="{{route('delete_employee_category',$emp_type->id)}}"><i class="fa-solid fa-trash"></i></a>&nbsp &nbsp
                                        </td>
                                    </tr>
